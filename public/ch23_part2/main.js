...
const domElements = {
    ...
    verseNavigation: document.getElementById('verse-navigation'),
...
function onWindowResize() {
    ...
    // Handle mobile panel visibility
    if (window.innerWidth < 768) {
        if (!domElements.contentPanel.classList.contains('collapsed')) {
            domElements.contentPanel.classList.add('visible');
        }
    } else {
        domElements.contentPanel.classList.remove('visible');
    }
}

function initEventListeners() {
    // Panel toggle
    domElements.panelToggle.addEventListener('click', togglePanel);

    // Collapsible sections
    domElements.explanationHeader.addEventListener('click', () => {
        toggleSection(domElements.explanationContent, domElements.explanationHeader.querySelector('.toggle-icon'));
    });

    domElements.controlsHeader.addEventListener('click', () => {
        toggleSection(domElements.controlsContent, domElements.controlsHeader.querySelector('.toggle-icon'));
    });
...
function togglePanel() {
    domElements.contentPanel.classList.toggle('collapsed');
}

function toggleSection(contentElement, iconElement) {
    contentElement.classList.toggle('collapsed');
    iconElement.textContent = contentElement.classList.contains('collapsed') ? '►' : '▼';
}

function createVerseNavigation() {
    // Clear any existing buttons
    domElements.verseNavigation.innerHTML = '';

    // Create a button for each verse
    verses.forEach((verse, index) => {
        const button = document.createElement('button');
        button.classList.add('verse-nav-btn');
        button.textContent = verse.id;
        button.addEventListener('click', () => updateVerseContent(index));
        domElements.verseNavigation.appendChild(button);
    });
}
...
function initPanel() {
    // Set default states
    if (window.innerWidth < 768) {
        // Mobile default - panel visible but sections collapsed
        domElements.contentPanel.classList.add('visible');
        domElements.explanationContent.classList.add('collapsed');
        domElements.explanationHeader.querySelector('.toggle-icon').textContent = '►';
    } else {
        // Desktop default - explanation expanded, controls collapsed
        domElements.controlsContent.classList.add('collapsed');
        domElements.controlsHeader.querySelector('.toggle-icon').textContent = '►';
    }
}
...
function updateVerseContent(index) {
    ...
    const buttons = domElements.verseNavigation.querySelectorAll('.verse-nav-btn');
