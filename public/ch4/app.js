...
const chapterTitle = document.getElementById('chapter-title'); // Now using this ID
const verseNavigation = document.getElementById('verse-navigation');
const verseNumber = document.getElementById('verse-number'); // Now inside verse-navigation
const verseText = document.getElementById('verse-text'); // Now inside explanation-section
...
// --- Animation Creation Functions (createFormCauseAnimation, etc.) ---
...
function setupEventListeners() {
    ...
    window.addEventListener('mousedown', (event) => {
        // Prevent interaction if clicking on the panel itself
    ...
    window.addEventListener('touchstart', (event) => {
        // Prevent interaction if touching the panel itself
        if (sidePanel.contains(event.target)) return;
        if (currentAnimation && currentAnimation.onInteractionStart) {
            currentAnimation.onInteractionStart(event);
        }
...
function updateVerseContent() {
    ...
    // Update content using correct IDs from the new HTML structure
    // chapterTitle is static in the HTML, no need to update here
    verseNumber.textContent = `Verse ${verse.number}`;
    verseText.innerHTML = verse.text; // Use innerHTML for <br> tags
    ...
    // Note: The default expanded/collapsed state is now handled by CSS.
    // If you need JS to override this based on verse change, add logic here.
    // Example: Force explanation open on verse change:
    // const explanationSection = document.getElementById('explanation-section');
    // if (explanationSection) explanationSection.classList.add('expanded');
    // const explanationIcon = explanationSection?.querySelector('.toggle-icon');
    // if (explanationIcon) explanationIcon.textContent = '▼';
