...
class MadhyamakaQuantumApp {
    ...
    setupUI() {
        ...
        if (explanationSection) {
            if (!isMobile) explanationSection.classList.add('section-expanded');
        }
        if (controlsSection) {
            if (!isMobile) controlsSection.classList.add('section-expanded');
        }
    }
    ...
    loadVerse(index) {
        ...
        if (verseContent) {
            verseContent.innerHTML = `
                <div id="verse-text">${verse.text}</div>
                <div class="section">
        ...
        // Update title
        document.querySelector('#verse-details h1').textContent = `Chapter 21: Investigation of Rising and Passing - Verse ${verse.number}`;
