...
function initUI() {
    ...
    hideControlsBtn.addEventListener('click', () => {
        ...
        hideControlsBtn.textContent = controlsPanel.classList.contains('controls-hidden') 
            ? 'Show Controls' 
            : 'Hide Controls';
    });
    
    hideTextBtn.addEventListener('click', () => {
        textPanel.classList.toggle('text-hidden');
        hideTextBtn.textContent = textPanel.classList.contains('text-hidden') 
            ? 'Show Text' 
            : 'Hide Text';
    });
...
function loadVerse(verseIndex) {
    ...
    // Update UI elements
    verseNumberEl.textContent = `Verse ${verseData.number}/12`;
...
function createVerseFiveAnimation() {
    ...
    const energyIndicatorMaterial = new THREE.MeshBasicMaterial({
        color: config.colorScheme.primary,
        transparent: true,
        opacity: 0.5,
...
function createVerseNineAnimation() {
    ...
    const alphaParticleMaterial = new THREE.MeshBasicMaterial({
        color: 0xffaa00,
        emissive: 0xffaa00,
        emissiveIntensity: 1,
        transparent: true,
