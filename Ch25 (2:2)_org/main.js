...
// Expose currentVerse and navigateToVerse to window object for access from panel-ui.js
window.currentVerse = currentVerse;
window.navigateToVerse = navigateToVerse;
window.verseData = verseData;
...
function initCurrentVerse() {
    ...
    if (document.getElementById('verse-title-overlay')) {
        document.getElementById('verse-title-overlay').textContent = verse.title;
    }
    
    verseContent.textContent = verse.content;
    if (document.getElementById('verse-content-overlay')) {
        document.getElementById('verse-content-overlay').textContent = verse.content;
    }
    
    verseExplanation.textContent = verse.explanation;
    if (document.getElementById('verse-explanation-overlay')) {
        document.getElementById('verse-explanation-overlay').textContent = verse.explanation;
    }
    
    verseNumber.textContent = `Verse ${currentVerse}/24`;
    if (document.getElementById('verse-number-overlay')) {
        document.getElementById('verse-number-overlay').textContent = `Verse ${currentVerse}/24`;
    }
    ...
    if (window.updatePanelVerseContent) {
        window.updatePanelVerseContent(currentVerse);
    }
}
...
function navigateToVerse(verseNum) {
    ...
    setTimeout(() => {
        ...
        window.currentVerse = currentVerse; // Update window reference
        initCurrentVerse();
...
// Sync panel control buttons with overlay buttons
function syncPanelControls() {
    ...
    document.getElementById('toggle-text').addEventListener('click', () => {
        ...
        if (textVisible) {
            textOverlay.classList.remove('hidden');
            document.getElementById('toggle-text').textContent = 'Hide Text';
            if (document.getElementById('toggle-text-overlay')) {
                document.getElementById('toggle-text-overlay').textContent = 'Hide Text';
            }
        } else {
            textOverlay.classList.add('hidden');
            document.getElementById('toggle-text').textContent = 'Show Text';
            if (document.getElementById('toggle-text-overlay')) {
                document.getElementById('toggle-text-overlay').textContent = 'Show Text';
            }
        }
    });
    ...
    document.getElementById('toggle-explanation').addEventListener('click', () => {
        ...
        if (explanationVisible) {
            ...
            if (document.getElementById('toggle-explanation-overlay')) {
                document.getElementById('toggle-explanation-overlay').textContent = 'Hide Explanation';
            }
        } else {
            verseExplanation.style.display = 'none';
            document.getElementById('toggle-explanation').textContent = 'Show Explanation';
            if (document.getElementById('toggle-explanation-overlay')) {
                document.getElementById('toggle-explanation-overlay').textContent = 'Show Explanation';
            }
        }
    });
}
...
function init() {
    ...
    // Make control buttons in panel work the same as overlay buttons
    syncPanelControls();
}
