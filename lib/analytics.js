...
function getAnonymousId() {
  ...
  if (!anonymousId) {
    anonymousId = `anon_${Math.random().toString(36).slice(2)}_${Date.now()}`;
...
function getSessionId() {
  ...
  if (!window.__mmk_session_id) {
    window.__mmk_session_id = `sess_${Math.random().toString(36).slice(2)}_${Date.now()}`;
  }
...
export function trackMonetizationEvent(eventName, properties = {}) {
  ...
  const missing = REQUIRED_MONETIZATION_PROPS.filter((key) => !normalized[key] && key !== 'anonymous_id' && key !== 'session_id');
